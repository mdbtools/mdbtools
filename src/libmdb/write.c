...
mdb_put_int16(void *buf, guint32 offset, guint32 value)
{
	...
	memcpy((uint8_t*)buf + offset, &value, 2);
}
void
...
{
	value = GINT32_TO_LE(value);
	memcpy((uint8_t*)buf + offset, &value, 4);
}
void
...
mdb_crack_row(MdbTableDef *table, int row_start, int row_end, MdbField *fields)
{
	...
	nullmask = (uint8_t*)pg_buf + row_end - bitmask_sz + 1;
	...
	for (i=0;i<table->num_cols;i++) {
		...
		if ((fields[i].is_fixed)
		 && (fixed_cols_found < row_fixed_cols)) {
			...
			fields[i].value = (uint8_t*)pg_buf + row_start + col_start;
...
mdb_add_row_to_pg(MdbTableDef *table, unsigned char *row_buffer, int new_row_size)
{
	...
	if (table->is_temp_table) {
		...
		for (i=0;i<num_rows;i++) {
			...
			pos -= row_size;
			memcpy((uint8_t*)new_pg + pos, mdb->pg_buf + row_start, row_size);
	...
	/* add our new row */
	pos -= new_row_size;
	memcpy((uint8_t*)new_pg + pos, row_buffer, new_row_size);
...
int i, pos;
	...
	/* our row */
	pos -= new_row_size;
	memcpy((uint8_t*)new_pg + pos, new_row, new_row_size);
...
*/
{
	...
	while (mdb_index_find_next_on_page(mdb, ipg)) {
		...
		memcpy((uint8_t*)new_pg + ipg->offset, mdb->pg_buf + ipg->offset, ipg->len);
		ipg->offset += ipg->len;
		ipg->len = 0;
	...
	memcpy((uint8_t*)new_pg + ipg->offset + 1, key_hash, col->col_size);
